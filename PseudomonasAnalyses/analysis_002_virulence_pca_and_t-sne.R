...
tsne_result <- Rtsne(tsne_result, dims = 2, perplexity = 18, check_duplicates = F,
                     max_iter = 5000)
...
ggplot(tsne_dataFrame, aes(X, Y, color = phylogroup)) +
  geom_point() +  
  geom_text(aes(label = ST),hjust = 0, vjust = 0, nudge_x = 0.03,
                nudge_y = 0.03) +
  scale_color_manual(values = my_colors)+
  theme_classic()+
  ggtitle("Virulence t-sne labelled by phylogroup, with perplexity = 18 ") +
  theme(plot.title = element_text(hjust = 0.5))


# Generate a list of 55 distinct colors
my_colors <- c("chocolate", "blue4","black", "gold","chocolate3",
              "wheat3",
              "turquoise" , "green1", "magenta","khaki4", "darkred","azure4",
          "grey12","sienna1" , "peachpuff4"  , "tomato1"  ,
             "violetred4" ,  "lawngreen", "gray47")

# Display the first 55 colors in the list
#print(my_colors)
# Generate a list of 55 distinct colors
#my_colors <- colors()[1:1000]
